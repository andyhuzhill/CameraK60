...
//中断发送部分
extern  uint8 RX_ISR_FIFO[RX_ISR_FIFO_PACKET][MAX_ONCE_TX_NUM];
...
//extern  uint8 NRF_Check(void);
extern  uint8 NRF_Tx_Dat(uint8 *txbuf,uint32 len);               //len 必须是 MAX_ONCE_TX_NUM 整数倍，剩下不足32位的，数据无效
extern  uint8 NRF_Rx_Dat(uint8 *rxbuf);                       //每次读取 MAX_ONCE_TX_NUM 个字节（返回接收结果：成功或者失败）




extern  void    NRF_Handler(void);
extern  void    NRF_ISR_Tx_Dat(uint8 *txbuf,uint32 len);      //中断方式发送数据
extern  uint8      NRF_ISR_Tx_State(void);                 //查询发送状态

extern  uint8      NRF_ISR_Rx_Dat(uint8 *rxbuf);              //每次读取 MAX_ONCE_TX_NUM 个字节（返回接收结果：成功或者失败，成功了，还有不断检查是否有数据接收到）
