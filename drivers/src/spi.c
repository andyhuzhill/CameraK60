...
//功能概要：SPI发送数据。
//=========================================================================
void spi_send(SPIn spin,uint8 data[],uint32 len)
{
    uint32 i = 0;
    uint8 temp;
    SPI_TX_WAIT(spin);
...
//功能概要：SPI发送数据。
//=========================================================================
void spi_WR(SPIn spin,uint8* data,uint32 len)
{
    uint32 i = 0;
    ...
    for(i = 0;i < (len-1);i++)
    {
        ...
        while( !(SPI_SR_REG(SPIN[spin]) & SPI_SR_RFDF_MASK));    //RFDF为1，Rx FIFO is not empty.
        data[i] = (uint8)SPI_POPR_REG(SPIN[spin]);    //保存接收到的数据 
        SPI_SR_REG(SPIN[spin]) |= SPI_SR_RFDF_MASK;
    }
...
//功能概要：SPI接收数据。
//=========================================================================
uint32 spi_re_max(SPIn spin,uint8 data[],uint32 maxlen)
{
    uint32   n=0;

    while(SPI_SR_REG(SPIN[spin]) & SPI_SR_RFDF_MASK)    //RFDF为1，Rx FIFO is not empty.
    {
        data[n++] = (uint8)SPI_POPR_REG(SPIN[spin]);    //保存接收到的数据            
        SPI_SR_REG(SPIN[spin]) |= SPI_SR_RFDF_MASK;
        if(n>=maxlen)
        {
            break;
...
//功能概要：SPI接收数据。
//=========================================================================
uint32 spi_re(SPIn spin,uint8 data[])
{
    uint32   n=0;
