...
typedef	nrf_result_e (* NRF_PFUNC)(uint8 *);               //定义函数指针，用于保存命令控制的
...
nrf_result_e NRF_MSG_send(crtl_e  com,uint8 * sendbuf)
{
    ...
    //执行对应的发送函数
    result = (tx_com_table[(uint8)com] )(sendbuf);  //对应的发送函数要注意前面两个字节不能用
...
nrf_result_e  tx_com_img(uint8 * sendbuf)
{
    ...
    sendbuf[CAMERA_SIZE + 2*COM_LEN -2]=(uint8)~COM_IMG;         //在尾部写入结束传输标记
...
{
    nrf_result_e result;
    uint8  status;	//用于判断接收/发送状态
    uint8  flag = 0;   //但接收到数据有误，就标记为1
    uint8  rxflag  = 0;//标记是否有接收到数据
    crtl_e  comtmp;
    do
    {
        flag = 0;
