...
static  uint8      NRF_ReadReg(uint8 reg);

static  uint8      NRF_ReadBuf(uint8 reg,uint8 *pBuf,uint8 len);
static  uint8      NRF_WriteBuf(uint8 reg ,uint8 *pBuf,uint32 len);
static  uint8      NRF_RW(uint8 dat);
static  void    NRF_ISR_Tx_Handler(void);               //发送中断处理函数
static  void    NRF_ISR_Rx_Handler(void);
static  uint8      NRF_Check(void);

volatile uint8     nrf_mode=NOT_INIT;
volatile uint8 *   isr_addr   =NULL;
volatile uint32    isr_L      =0;     //L = len / MAX_ONCE_TX_NUM
volatile uint8     isr_state  =0;

volatile uint8     front = 0,rear = 0; //接收FIFO的指针
volatile uint8     re_flag = QUEUE_EMPTY;
...
uint8 RX_ISR_FIFO[RX_ISR_FIFO_PACKET][MAX_ONCE_TX_NUM];      //中断接收的FIFO
...
void NRF_Init(void)
{
    ...
    gpio_init(PORT_E,28, Mode_OUT, State_Low);           //初始化CE，默认进入待机模式
    //gpio_init(PORTE,27, GPI,LOW);           //初始化IRQ管脚为输入 
    gpio_init(PORT_A,14, Mode_OUT, State_High);          //初始化PCSN管脚为输出，低电平选中  
 ...
 * 调用  ：外部调用
 */
uint8 NRF_ReadBuf(uint8 reg,uint8 *pBuf,uint8 len)
{
...
#define NRF_CHECH_DATA  0xC2
	uint8 buf[5]={NRF_CHECH_DATA,NRF_CHECH_DATA,NRF_CHECH_DATA,NRF_CHECH_DATA,NRF_CHECH_DATA};
	uint8 buf1[5];
	uint8 i; 
...
//传输一次，数据最长为 32字节
static uint8 NRF_TX_Dat_Once(uint8 *txbuf)
{
	uint8 state;
 ...
 * 调用  ：外部调用
 */ 
uint8 NRF_Tx_Dat(uint8 *txbuf,uint32 len)
{
    uint8 state;  
    uint32 L;
 ...
 * 调用  ：外部调用
 */ 
void NRF_ISR_Tx_Dat(uint8 *txbuf,uint32 len)
{
    uint32 L = (len+ MAX_ONCE_TX_NUM -1)/MAX_ONCE_TX_NUM;
...
void nrf_cpy( uint8 * dst, uint8 * src, uint32 count)
{
      uint32 n = (count + 7 ) / 8 ;
      switch (count % 8 ) {
      case 0 :   do {   * dst ++ = * src ++ ;
...
void NRF_ISR_Tx_Dat_Once(void)
{
    if(isr_L > 0)
    {
        ...
        /*写数据到TX BUF 最大 32个字节*/						
        NRF_WriteBuf(WR_TX_PLOAD,(uint8 *)isr_addr,MAX_ONCE_TX_NUM);
...
void NRF_ISR_Rx_Handler(void)
{
	...
	if(state & RX_DR)                                 //接收到数据
	{
	    NRF_ReadBuf(RD_RX_PLOAD,(uint8 *)&(RX_ISR_FIFO[rear]),RX_PLOAD_WIDTH);  //读取数据
	    NRF_WriteReg(FLUSH_RX,NOP);                     //清除RX FIFO寄存器
...
uint8  NRF_ISR_Rx_Dat(uint8 *rxbuf)
{
    ...
    //复制数据
    nrf_cpy(rxbuf,(uint8 *)&(RX_ISR_FIFO[front]),MAX_ONCE_TX_NUM);
